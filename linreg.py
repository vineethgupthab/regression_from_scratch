...
def loss_ridge(X, y, B, lmbda):
    return np.sum(np.power((y-np.dot(X, B)),2))+lmbda*np.sum(np.power(B,2))
...
def log_likelihood(X, y, B,lmbda):
    sig = np.log((1/sigmoid(np.dot(X,B)))/(1-(1/sigmoid(np.dot(X,B)))))
    return np.sum(np.dot(y,sig) - np.log(1+np.exp(sig)))

def log_likelihood_gradient(X, y, B, lmbda):
    sig = sigmoid(np.dot(X,B))
    return np.dot(-X.T,(y-sig))
...
def minimize(X, y, loss_gradient,
    ...
    while i<max_iter:
        ...
        h+=gradient**2
        if (np.linalg.norm(gradient)+lmbda)<=precision:
            return B
...
class RidgeRegression621: # MODIFY THIS ONE
    ...
    def fit(self, X, y):
        ...
        B0 = np.mean(y) - np.mean(np.dot(X,self.B))
